...
backup:
    ...
    @if [ -f SafeRelay.xcodeproj/project.pbxproj ]; then cp SafeRelay.xcodeproj/project.pbxproj SafeRelay.xcodeproj/project.pbxproj.backup; fi
    @if [ -f SafeRelay/Info.plist ]; then cp SafeRelay/Info.plist SafeRelay/Info.plist.backup; fi
...
restore:
    ...
    @if [ -f SafeRelay/LaunchScreen.storyboard.ios ]; then mv SafeRelay/LaunchScreen.storyboard.ios SafeRelay/LaunchScreen.storyboard; fi
    @# Use git to restore all modified files except Justfile
    @git checkout -- project.yml SafeRelay.xcodeproj/project.pbxproj SafeRelay/Info.plist 2>/dev/null || echo "⚠️  Could not restore some files with git"
    @# Remove any backup files
    @rm -f SafeRelay.xcodeproj/project.pbxproj.backup SafeRelay/Info.plist.backup 2>/dev/null || true
...
patch-for-macos: backup
    ...
    @if [ -f SafeRelay/LaunchScreen.storyboard ]; then mv SafeRelay/LaunchScreen.storyboard SafeRelay/LaunchScreen.storyboard.ios; fi

# Build the macOS app
build: #check generate
    @echo "Building BitChat for macOS..."
    @xcodebuild -project SafeRelay.xcodeproj -scheme "SafeRelay (macOS)" -configuration Debug CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED=NO CODE_SIGN_ENTITLEMENTS="" build

# Run the macOS app
run: build
    @echo "Launching BitChat..."
    @find ~/Library/Developer/Xcode/DerivedData -name "SafeRelay.app" -path "*/Debug/*" -not -path "*/Index.noindex/*" | head -1 | xargs -I {} open "{}"

# Clean build artifacts and restore original files
clean: restore
    @echo "Cleaning build artifacts..."
    @rm -rf ~/Library/Developer/Xcode/DerivedData/SafeRelay-* 2>/dev/null || true
    @# Only remove the generated project if we have a backup, otherwise use git
    @if [ -f SafeRelay.xcodeproj/project.pbxproj.backup ]; then \
        rm -rf SafeRelay.xcodeproj; \
    else \
        git checkout -- SafeRelay.xcodeproj/project.pbxproj 2>/dev/null || echo "⚠️  Could not restore project.pbxproj"; \
    fi
...
dev-run: check
    ...
    @xcodebuild -project SafeRelay.xcodeproj -scheme "SafeRelay_macOS" -configuration Debug CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED=NO CODE_SIGN_ENTITLEMENTS="" build
    @find ~/Library/Developer/Xcode/DerivedData -name "SafeRelay.app" -path "*/Debug/*" -not -path "*/Index.noindex/*" | head -1 | xargs -I {} open "{}"
...
nuke:
    ...
    @rm -rf ~/Library/Developer/Xcode/DerivedData/SafeRelay-* 2>/dev/null || true
    @rm -rf SafeRelay.xcodeproj 2>/dev/null || true
    @rm -f SafeRelay.xcodeproj/project.pbxproj.backup 2>/dev/null || true
    @rm -f SafeRelay/Info.plist.backup 2>/dev/null || true
    @# Restore iOS-specific files if they were moved
    @if [ -f SafeRelay/LaunchScreen.storyboard.ios ]; then mv SafeRelay/LaunchScreen.storyboard.ios SafeRelay/LaunchScreen.storyboard; fi
    @git checkout SafeRelay.xcodeproj/project.pbxproj SafeRelay/Info.plist 2>/dev/null || echo "⚠️  Not a git repo or no changes to restore"
