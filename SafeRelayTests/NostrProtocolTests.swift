...
struct NostrProtocolTests {
    ...
    func testAckRoundTripNIP44V2_Delivered() throws {
        ...
        #expect(content.hasPrefix("SafeRelay1:"))
        let base64url = String(content.dropFirst("SafeRelay1:".count))
        let packetData = try #require(Self.base64URLDecode(base64url))
        let packet = try #require(SafeRelayPacket.from(packetData), "Failed to decode SafeRelay packet")
