...
struct FragmentationTests {
    ...
    func maxSizedFileTransferSurvivesReassembly() async throws {
        ...
        let filePacket = SafeRelayFilePacket(
            fileName: "limit.bin",
...
extension FragmentationTests {
    ...
    private final class CaptureDelegate: SafeRelayDelegate, @unchecked Sendable {
        private let lock = NSLock()
        private var _publicMessages: [(peerID: PeerID, nickname: String, content: String)] = []
        private var _receivedMessages: [SafeRelayMessage] = []
        ...
        var receivedMessages: [SafeRelayMessage] {
            lock.lock()
            defer { lock.unlock() }
    ...
    private func makeLargePublicPacket(senderShortHex: PeerID, size: Int) -> SafeRelayPacket {
    ...
    private func fragmentPacket(_ packet: SafeRelayPacket, fragmentSize: Int, fragmentID: Data? = nil, pad: Bool = true) -> [SafeRelayPacket] {
