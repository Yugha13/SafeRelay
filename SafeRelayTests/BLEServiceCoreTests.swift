...
private func makePublicPacket(content: String, sender: PeerID, timestamp: UInt64) -> SafeRelayPacket {
    SafeRelayPacket(
...
private final class PublicCaptureDelegate: SafeRelayDelegate {
    private let lock = NSLock()
    private(set) var publicMessages: [SafeRelayMessage] = []
    ...
    func publicMessagesSnapshot() -> [SafeRelayMessage] {
        lock.lock()
        defer { lock.unlock() }
