...
struct ContentView: View {
    ...
    @State private var sosHoldDuration: TimeInterval = 0
    @State private var sosHoldTimer: Timer?
    @State private var sosStartDate: Date?
    let requiredSOSHoldTime: TimeInterval = 3.0
    ...
    private var textColor: Color {
        colorScheme == .dark ? Color(red: 1.0, green: 0.3, blue: 0.3) : Color(red: 0.8, green: 0, blue: 0)
    }

    private var secondaryTextColor: Color {
        colorScheme == .dark ? Color(red: 1.0, green: 0.3, blue: 0.3).opacity(0.8) : Color(red: 0.8, green: 0, blue: 0).opacity(0.8)
    }
...
// MARK: - Body
    ...
    private func messagesView(privatePeer: PeerID?, isAtBottom: Binding<Bool>) -> some View {
        ...
        let windowedMessages: [SafeRelayMessage] = Array(messages.suffix(currentWindowCount))
    ...
    private var inputView: some View {
        VStack(alignment: .leading, spacing: 6) {
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ForEach(Array(viewModel.autocompleteSuggestions.prefix(4)), id: \.self) { suggestion in
                        Button(action: {
                            ...
                            HStack {
                                Text(suggestion)
                                    .font(.SafeRelaySystem(size: 11, design: .monospaced))
                                    .foregroundColor(textColor)
                                    .fontWeight(.medium)
                                Spacer()
                            }
            ...
            HStack(alignment: .center, spacing: 4) {
                ...
                .font(.SafeRelaySystem(size: 15, design: .monospaced))
                .foregroundColor(textColor)
    ...
    private func handleOpenURL(_ url: URL) {
        guard url.scheme == "SafeRelay" else { return }
        switch url.host {
        case "user":
            ...
            VStack(alignment: .leading, spacing: 8) {
                HStack(spacing: 12) {
                    Text(peopleSheetTitle)
                        .font(.SafeRelaySystem(size: 18, design: .monospaced))
                        .foregroundColor(textColor)
                    Spacer()
                    Button(action: { showVerifySheet = true }) {
                        Image(systemName: "qrcode")
                            .font(.SafeRelaySystem(size: 14))
                    }
                    .buttonStyle(.plain)
                    .help(
                    ...
                    Button(action: {
                        ...
                        Image(systemName: "xmark")
                            .font(.SafeRelaySystem(size: 12, weight: .semibold, design: .monospaced))
                ...
                Text(activeText)
                    .font(.SafeRelaySystem(size: 12, design: .monospaced))
        ...
        Button(action: {
            ...
            HStack(spacing: 6) {
                ...
                Text(context.displayName)
                    .font(.SafeRelaySystem(size: 16, weight: .medium, design: .monospaced))
                    .foregroundColor(textColor)
    ...
    private var mainHeaderView: some View {
        HStack(spacing: 0) {
            Text(verbatim: "SafeRelay/")
                .font(.SafeRelaySystem(size: 18, weight: .medium, design: .monospaced))
                .foregroundColor(textColor)
                .onTapGesture(count: 3) {
            ...
            HStack(spacing: 10) {
                // Map button
                Button(action: { showDisasterMap = true }) {
            ...
            #if os(iOS)
            .fullScreenCover(isPresented: $showDisasterMap) {
                DisasterMapView()
            }
            #else
            .sheet(isPresented: $showDisasterMap) {
                DisasterMapView()
...
private extension ContentView {
    func mediaAttachment(for message: SafeRelayMessage) -> MessageMedia? {
    ...
    func mediaSendState(for message: SafeRelayMessage, mediaURL: URL) -> (isSending: Bool, progress: Double?, canCancel: Bool) {
        var isSending = false
        var progress: Double?
    ...
    @ViewBuilder
    private func messageRow(for message: SafeRelayMessage) -> some View {
        if message.sender == "system" {
    ...
    @ViewBuilder
    private func systemMessageRow(_ message: SafeRelayMessage) -> some View {
    ...
    @ViewBuilder
    private func mediaMessageRow(message: SafeRelayMessage, media: MessageMedia) -> some View {
    ...
    private func expandWindow(ifNeededFor message: SafeRelayMessage,
                              allMessages: [SafeRelayMessage],
                              privatePeer: PeerID?,
    ...
    private var sosButtonView: some View {
        let progress = min(sosHoldDuration / requiredSOSHoldTime, 1.0)
        let isComplete = progress >= 1.0
        
        return ZStack {
            // Background fill during hold
            Circle()
                .fill(Color.red.opacity(isHoldingSOS ? 0.3 : 0.0))
                .scaleEffect(isHoldingSOS ? 1.2 : 1.0)
                .animation(.easeInOut(duration: 0.2), value: isHoldingSOS)
            ...
            Image(systemName: "exclamationmark.triangle.fill")
                .font(.SafeRelaySystem(size: isHoldingSOS ? 20 : 22))
                .foregroundColor(isHoldingSOS ? .red : .red.opacity(0.8))
                .animation(.spring(response: 0.3, dampingFraction: 0.6), value: isHoldingSOS)
        }
        .frame(width: 36, height: 36)
        .contentShape(Circle())
        .gesture(
            DragGesture(minimumDistance: 0)
                ...
                .onEnded { _ in
                    ...
                    cancelSOSHold()
                    if wasComplete && shouldShowMediaControls { // basic filter to check if acceptable in this channel
                        viewModel.sendSOS()
                    }
                }
        )
        .accessibilityLabel("SOS Emergency")
        .accessibilityHint("Hold for \(Int(requiredSOSHoldTime)) seconds to broadcast emergency SOS")
    }
    
    private func startSOSHold() {
        isHoldingSOS = true
        sosHoldDuration = 0
        sosStartDate = Date()
        ...
        sosHoldTimer?.invalidate()
        sosHoldTimer = Timer.scheduledTimer(withTimeInterval: 0.05, repeats: true) { _ in
            if let start = sosStartDate {
                let duration = Date().timeIntervalSince(start)
                sosHoldDuration = duration
                
                if duration >= requiredSOSHoldTime {
                    // Maximum hold reached
                    sosHoldTimer?.invalidate()
                    sosHoldDuration = requiredSOSHoldTime
        ...
        if let timer = sosHoldTimer {
            RunLoop.main.add(timer, forMode: .common)
        }
    }
    
    private func cancelSOSHold() {
        isHoldingSOS = false
        sosHoldTimer?.invalidate()
        sosHoldTimer = nil
        sosStartDate = nil
                ...
                HStack {
                    Button(action: { dismiss() }) {
                        Text("close", comment: "Button to dismiss fullscreen media viewer")
                            .font(.SafeRelaySystem(size: 15, weight: .semibold))
                            .foregroundColor(.white)
                            .padding(.horizontal, 16)
                            .padding(.vertical, 8)
