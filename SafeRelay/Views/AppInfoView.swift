                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    Button(action: { dismiss() }) {
                        Image(systemName: "xmark")
                            .font(.SafeRelaySystem(size: 13, weight: .semibold, design: .monospaced))
                            .foregroundColor(textColor)
        ...
        VStack(alignment: .leading, spacing: 24) {
            ...
            VStack(alignment: .center, spacing: 8) {
                Text(Strings.appName)
                    .font(.SafeRelaySystem(size: 32, weight: .bold, design: .monospaced))
                    .foregroundColor(textColor)
                
                Text(Strings.tagline)
                    .font(.SafeRelaySystem(size: 16, design: .monospaced))
...
struct SectionHeader: View {
    ...
    var body: some View {
        Text(title)
            .font(.SafeRelaySystem(size: 16, weight: .bold, design: .monospaced))
            .foregroundColor(textColor)
            .padding(.top, 8)
    }
}
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text(info.title)
                    .font(.SafeRelaySystem(size: 14, weight: .semibold, design: .monospaced))
                    .foregroundColor(textColor)
