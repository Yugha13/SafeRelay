...
final class SafeRelayMessage: Codable {
    enum MessageType: UInt8, Codable {
        case normal = 0
        case sos = 1
        case broadcast = 2
        case resourceUpdate = 3
        case medicalRequest = 4
        case safeStatus = 5
    }

    enum PriorityLevel: UInt8, Codable {
        case critical = 1
        case urgent = 2
        case info = 3
    }
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case isPrivate, recipientNickname, senderPeerID, mentions, deliveryStatus, messageType, priorityLevel, geoLocation
    }
...
extension SafeRelayMessage: Equatable {
    static func == (lhs: SafeRelayMessage, rhs: SafeRelayMessage) -> Bool {
...
extension SafeRelayMessage {
    func toBinaryPayload() -> Data? {
        ...
        // Message format:
        // - Flags: 1 byte (bit 0: isRelay, bit 1: isPrivate, bit 2: hasOriginalSender, bit 3: hasRecipientNickname, bit 4: hasSenderPeerID, bit 5: hasMentions, bit 6: hasGeoLocation)
        ...
        // - Mentions array
        // - GeoLocation (16 bytes: 8 bytes lat + 8 bytes lon)
        ...
        data.append(priorityLevel.rawValue)
        ...
        if let geo = geoLocation {
            withUnsafeBytes(of: geo.latitude.bitPattern.bigEndian) { data.append(contentsOf: $0) }
            withUnsafeBytes(of: geo.longitude.bitPattern.bigEndian) { data.append(contentsOf: $0) }
        }
    ...
    convenience init?(_ data: Data) {
        ...
        let hasGeoLocation = (flags & 0x40) != 0
        ...
        let messageType = MessageType(rawValue: dataCopy[offset]) ?? .normal
        offset += 1
        
        // Priority Level
        guard offset < dataCopy.count else { return nil }
        let priorityLevel = PriorityLevel(rawValue: dataCopy[offset]) ?? .info
        offset += 1
        ...
        if hasGeoLocation && offset + 16 <= dataCopy.count {
            let latBytes = Array(dataCopy[offset..<offset+8])
            let lonBytes = Array(dataCopy[offset+8..<offset+16])
            
            let latInt = latBytes.reduce(0) { result, byte in (result << 8) | UInt64(byte) }
            let lonInt = lonBytes.reduce(0) { result, byte in (result << 8) | UInt64(byte) }
            
            geoLocation = GeoLocation(
                latitude: Double(bitPattern: latInt),
                longitude: Double(bitPattern: lonInt)
            )
            offset += 16
        }
...
extension Array where Element == SafeRelayMessage {
