...
struct RelayController {
    static func decide(ttl: UInt8,
                       ...
                       priorityLevel: SafeRelayMessage.PriorityLevel = .info) -> RelayDecision {
        ...
        // Critical SOS messages get maximum TTL and near-zero delay
        if priorityLevel == .critical {
            let delayMs = Int.random(in: 1...5)
            return RelayDecision(shouldRelay: true, newTTL: ttlCap, delayMs: delayMs)
        }
