...
final class UnifiedPeerService: ObservableObject, TransportPeerEventsDelegate {
    ...
    @Published private(set) var peers: [SafeRelayPeer] = []
    @Published private(set) var connectedPeerIDs: Set<PeerID> = []
    @Published private(set) var favorites: [SafeRelayPeer] = []
    @Published private(set) var mutualFavorites: [SafeRelayPeer] = []
    ...
    private func updatePeers() {
        ...
        var enrichedPeers: [SafeRelayPeer] = []
        ...
        enrichedPeers.sort { lhs, rhs in
            ...
            func rank(_ p: SafeRelayPeer) -> Int { p.isConnected ? 2 : (p.isReachable ? 1 : 0) }
        ...
        var favoritesList: [SafeRelayPeer] = []
        var mutualsList: [SafeRelayPeer] = []
        var newIndex: [PeerID: SafeRelayPeer] = [:]
    ...
    /// Get peer by ID
    func getPeer(by peerID: PeerID) -> SafeRelayPeer? {
    ...
    var allPeers: [SafeRelayPeer] { peers }
