...
/// - Emits events exclusively via `SafeRelayDelegate` for UI.
...
final class BLEService: NSObject {
    ...
    private static let centralRestorationID = "chat.SafeRelay.ble.central"
    private static let peripheralRestorationID = "chat.SafeRelay.ble.peripheral"
    ...
    private var pendingDirectedRelays: [PeerID: [String: (packet: SafeRelayPacket, enqueuedAt: Date)]] = [:]
    ...
    func sendMessage(_ content: String, mentions: [String] = [], to recipientID: PeerID? = nil, messageID: String? = nil, timestamp: Date? = nil, priority: SafeRelayMessage.PriorityLevel = .info, messageType: SafeRelayMessage.MessageType = .normal) {
        // Call directly if already on messageQueue, otherwise dispatch
        if DispatchQueue.getSpecific(key: messageQueueKey) == nil {
            messageQueue.async { [weak self] in
                self?.sendMessage(content, mentions: mentions, to: recipientID, messageID: messageID, timestamp: timestamp, priority: priority, messageType: messageType)
            }
            return
        }
        ...
        if let recipientID {
            let nick = (try? idBridge.getCurrentNostrIdentity()?.npub) ?? "Unknown"
            sendPrivateMessage(content, to: recipientID, recipientNickname: nick, messageID: messageID ?? UUID().uuidString, priority: priority, messageType: messageType)
            return
        }
        ...
        let basePacket = SafeRelayPacket(
    ...
    weak var delegate: SafeRelayDelegate?
    ...
    func stopServices() {
        ...
        let leavePacket = SafeRelayPacket(
    ...
    func sendMessage(_ content: String, mentions: [String], messageID: String, timestamp: Date, priority: SafeRelayMessage.PriorityLevel, messageType: SafeRelayMessage.MessageType) {
        sendMessage(content, mentions: mentions, to: nil, messageID: messageID, timestamp: timestamp, priority: priority, messageType: messageType)
    }
    
    func sendPrivateMessage(_ content: String, to peerID: PeerID, recipientNickname: String, messageID: String, priority: SafeRelayMessage.PriorityLevel, messageType: SafeRelayMessage.MessageType) {
        // Call directly if already on messageQueue
        if DispatchQueue.getSpecific(key: messageQueueKey) == nil {
            messageQueue.async { [weak self] in
                self?.sendPrivateMessage(content, to: peerID, recipientNickname: recipientNickname, messageID: messageID, priority: priority, messageType: messageType)
            }
            return
            let targetID = peerID.toShort()
            guard let recipientData = Data(hexString: targetID.id) else {
                SecureLogger.error("❌ Invalid recipient peer ID: \(peerID)", category: .session)
                return
            }

            var packet = SafeRelayPacket(
                type: messageType.rawValue,
            ...
            SecureLogger.debug("🔒 Routing private message: type=\(messageType.rawValue), priority=\(priority.rawValue), to \(peerID.id.prefix(8))…", category: .session)
            self.broadcastPacket(packet)
        }
    }
    
    func sendFileBroadcast(_ filePacket: SafeRelayFilePacket, transferId: String) {
        messageQueue.async { [weak self] in
            guard let self = self else { return }
    ...
    func sendFilePrivate(_ filePacket: SafeRelayFilePacket, to peerID: PeerID, transferId: String) {
        messageQueue.async { [weak self] in
            guard let self = self else { return }
            guard let payload = filePacket.encode() else {
    ...
    private func validatePacket(_ packet: SafeRelayPacket, from peerID: PeerID, connectionSource: ConnectionSource = .unknown) -> Bool {
    ...
    private func broadcastPacket(_ packet: SafeRelayPacket, transferId: String? = nil) {
        // Apply route if recipient exists (centralized route application)
        let packetToSend: SafeRelayPacket
    ...
    private func sendEncrypted(_ packet: SafeRelayPacket, data: Data, pad: Bool) {
    ...
    private func sendGenericBroadcast(_ packet: SafeRelayPacket, data: Data, pad: Bool) {
    ...
    private func sendOnAllLinks(packet: SafeRelayPacket, data: Data, pad: Bool, directedOnlyPeer: PeerID?) {
    ...
    private func sendPacketDirected(_ packet: SafeRelayPacket, to peerID: PeerID) {
    ...
    private func spoolDirectedPacket(_ packet: SafeRelayPacket, recipientPeerID: PeerID) {
    ...
    private func flushDirectedSpool() {
        ...
        let toSend: [(String, SafeRelayPacket)] = collectionsQueue.sync(flags: .barrier) {
            var out: [(String, SafeRelayPacket)] = []
            let now = Date()
    ...
    private func handleFileTransfer(_ packet: SafeRelayPacket, from peerID: PeerID) {
        ...
        guard let filePacket = SafeRelayFilePacket.decode(packet.payload) else {
    ...
    private func handleLeave(_ packet: SafeRelayPacket, from peerID: PeerID) {
...
extension BLEService: GossipSyncManager.Delegate {
    func sendPacket(_ packet: SafeRelayPacket) {
        broadcastPacket(packet)
    }

    func sendPacket(to peerID: PeerID, packet: SafeRelayPacket) {
        sendPacketDirected(packet, to: peerID)
    }

    func signPacketForBroadcast(_ packet: SafeRelayPacket) -> SafeRelayPacket {
...
extension BLEService {
    func _test_handlePacket(_ packet: SafeRelayPacket, fromPeerID: PeerID, preseedPeer: Bool = true) {
        if preseedPeer {
...
extension BLEService: CBPeripheralDelegate {
    ...
    private func processNotificationPacket(_ packet: SafeRelayPacket, from peripheral: CBPeripheral, peripheralUUID: String) {
...
extension BLEService {
    ...
    private func applyRouteIfAvailable(_ packet: SafeRelayPacket, to recipient: PeerID) -> SafeRelayPacket {
        ...
        let routedPacket = SafeRelayPacket(
            type: packet.type,
    ...
    private func forwardAlongRouteIfNeeded(_ packet: SafeRelayPacket) -> Bool {
    ...
    private func makeMessageID(for packet: SafeRelayPacket) -> String {
    ...
    private func priority(for packet: SafeRelayPacket, data: Data) -> OutboundPriority {
    ...
    private func sendFragmentedPacket(_ packet: SafeRelayPacket, pad: Bool, maxChunk: Int? = nil, directedOnlyPeer: PeerID? = nil, transferId: String? = nil) {
    ...
    private func startFragmentedPacket(_ context: PendingFragmentTransfer) {
        ...
        for (index, fragment) in fragments.enumerated() {
            ...
            let fragmentPacket = SafeRelayPacket(
    ...
    private func handleFragment(_ packet: SafeRelayPacket, from peerID: PeerID) {
        if DispatchQueue.getSpecific(key: messageQueueKey) != nil {
    ...
    private func _handleFragment(_ packet: SafeRelayPacket, from peerID: PeerID) {
    ...
    private func handleReceivedPacket(_ packet: SafeRelayPacket, from peerID: PeerID) {
        // Call directly if already on messageQueue, otherwise dispatch
        if DispatchQueue.getSpecific(key: messageQueueKey) == nil {
            messageQueue.async { [weak self] in
        ...
        do {
            ...
            let decision = RelayController.decide(
                ...
                priorityLevel: packet.priorityLevel
            )
    ...
    private func handleAnnounce(_ packet: SafeRelayPacket, from peerID: PeerID) {
    ...
    private func handleRequestSync(_ packet: SafeRelayPacket, from peerID: PeerID) {
    ...
    private func handleMessage(_ packet: SafeRelayPacket, from peerID: PeerID) {
    ...
    private func handleNoiseHandshake(_ packet: SafeRelayPacket, from peerID: PeerID) {
        ...
        if PeerID(hexData: packet.recipientID) == myPeerID {
            ...
            do {
                if let response = try noiseService.processHandshakeMessage(from: peerID, message: packet.payload) {
                    // Send response
                    let responsePacket = SafeRelayPacket(
                        type: MessageType.noiseHandshake.rawValue,
                        senderID: myPeerIDData,
                        recipientID: Data(hexString: peerID.id),
                        timestamp: UInt64(Date().timeIntervalSince1970 * 1000),
    ...
    private func handleNoiseEncrypted(_ packet: SafeRelayPacket, from peerID: PeerID) {
    ...
    private func performCleanup() {
        ...
        collectionsQueue.async(flags: .barrier) { [weak self] in
            ...
            if !self.pendingDirectedRelays.isEmpty {
                var cleaned: [PeerID: [String: (packet: SafeRelayPacket, enqueuedAt: Date)]] = [:]
