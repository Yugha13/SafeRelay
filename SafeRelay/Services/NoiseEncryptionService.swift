...
final class NoiseEncryptionService {
    ...
    // Thread safety
    private let serviceQueue = DispatchQueue(label: "chat.SafeRelay.noise.service", attributes: .concurrent)
    ...
    private func canonicalAnnounceBytes(peerID: Data, noiseKey: Data, ed25519Key: Data, nickname: String, timestampMs: UInt64) -> Data {
        var out = Data()
        // context
        let context = "SafeRelay-announce-v1".data(using: .utf8) ?? Data()
    ...
    /// Sign a SafeRelayPacket using the noise private key
    func signPacket(_ packet: SafeRelayPacket) -> SafeRelayPacket? {
    ...
    /// Verify a SafeRelayPacket signature using the provided public key
    func verifyPacketSignature(_ packet: SafeRelayPacket, publicKey: Data) -> Bool {
