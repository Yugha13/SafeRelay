...
final class MessageRouter {
    ...
    private struct QueuedMessage {
        ...
        let timestamp: Date
        let priorityLevel: SafeRelayMessage.PriorityLevel
    }
    ...
    func sendPrivate(_ content: String, to peerID: PeerID, recipientNickname: String, messageID: String) {
        if let transport = reachableTransport(for: peerID) {
            ...
            // Default level is info for now to match old behavior
            let message = QueuedMessage(content: content, nickname: recipientNickname, messageID: messageID, timestamp: Date(), priorityLevel: .info)
            outbox[peerID]?.append(message)
            
            // Sort by priority before eviction (lower is higher priority)
            outbox[peerID]?.sort(by: { $0.priorityLevel.rawValue < $1.priorityLevel.rawValue })

            // Enforce per-peer size limit with priority-based eviction
            if let count = outbox[peerID]?.count, count > Self.maxMessagesPerPeer {
                // Evict the lowest priority (highest raw value / last element)
                let evicted = outbox[peerID]?.removeLast()
                SecureLogger.warning("📤 Outbox overflow for \(peerID.id.prefix(8))… - evicted lowest priority message: \(evicted?.messageID.prefix(8) ?? "?")…", category: .session)
            }
