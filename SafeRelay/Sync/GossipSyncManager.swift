...
final class GossipSyncManager {
    ...
    private struct PacketStore {
        private(set) var packets: [String: SafeRelayPacket] = [:]
        private(set) var order: [String] = []

        mutating func insert(idHex: String, packet: SafeRelayPacket, capacity: Int) {
        ...
        func allPackets(isFresh: (SafeRelayPacket) -> Bool) -> [SafeRelayPacket] {
        ...
        mutating func remove(where shouldRemove: (SafeRelayPacket) -> Bool) {
        ...
        mutating func removeExpired(isFresh: (SafeRelayPacket) -> Bool) {
            remove { !isFresh($0) }
        }
    }
    ...
    private var latestAnnouncementByPeer: [PeerID: (id: String, packet: SafeRelayPacket)] = [:]
    ...
    func onPublicPacketSeen(_ packet: SafeRelayPacket) {
    ...
    private func isPacketFresh(_ packet: SafeRelayPacket) -> Bool {
    ...
    private func isAnnouncementFresh(_ packet: SafeRelayPacket) -> Bool {
    ...
    private func _onPublicPacketSeen(_ packet: SafeRelayPacket) {
    ...
    private func buildGcsPayload(for types: SyncTypeFlags) -> Data {
        var candidates: [SafeRelayPacket] = []
