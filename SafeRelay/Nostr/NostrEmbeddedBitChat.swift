...
struct NostrEmbeddedBitChat {
    /// Build a `SafeRelay1:` base64url-encoded BitChat packet carrying a private message for Nostr DMs.
    static func encodePMForNostr(content: String, messageID: String, recipientPeerID: PeerID, senderPeerID: PeerID) -> String? {
        ...
        return "SafeRelay1:" + base64URLEncode(data)
    }

    /// Build a `SafeRelay1:` base64url-encoded BitChat packet carrying a delivery/read ack for Nostr DMs.
    ...
    /// Build a `SafeRelay1:` ACK (delivered/read) without an embedded recipient peer ID (geohash DMs).
    ...
    /// Build a `SafeRelay1:` payload without an embedded recipient peer ID (used for geohash DMs).
