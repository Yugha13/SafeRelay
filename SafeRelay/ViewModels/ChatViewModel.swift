...
/// - **SafeRelayDelegate**: Receives messages and events from BLEService
...
/// implementing the SafeRelayDelegate protocol to handle network events.
final class ChatViewModel: ObservableObject, SafeRelayDelegate, CommandContextProvider, GeohashParticipantContext, MessageFormattingContext {
    ...
    @MainActor
    private func normalizedSenderKey(for message: SafeRelayMessage) -> String {
    ...
    @Published var messages: [SafeRelayMessage] = []
    ...
    @Published var allPeers: [SafeRelayPeer] = []
    var privateChats: [PeerID: [SafeRelayMessage]] {
    ...
    private var peerIndex: [PeerID: SafeRelayPeer] = [:]
    ...
    private let nicknameKey = "SafeRelay.nickname"
    ...
    init(
        ...
        let peersCancellable = unifiedPeerService.$peers
            ...
            .sink { [weak self] peers in
                ...
                var uniquePeers: [PeerID: SafeRelayPeer] = [:]
                for peer in peers {
    ...
    @MainActor
    func sendSOS() {
        // Collect location and battery data
        var locationText = ""
        if let location = LocationStateManager.shared.lastLocation {
            locationText = " GPS: \(location.coordinate.latitude), \(location.coordinate.longitude)"
        } else {
            locationText = " [GPS Unavailable]"
        }
        
        let batteryLevel = Int(LocationStateManager.shared.batteryLevel * 100)
        let batteryText = batteryLevel >= 0 ? " | Battery: \(batteryLevel)%" : ""
        
        let sosMessage = "⚠️ SOS ALERT ⚠️\(locationText)\(batteryText)"
        ...
        let message = SafeRelayMessage(
            ...
            geoLocation: LocationStateManager.shared.lastLocation.map { SafeRelayMessage.GeoLocation(latitude: $0.coordinate.latitude, longitude: $0.coordinate.longitude) }
        )
        
        timelineStore.append(message, to: activeChannel)
        refreshVisibleMessages(from: activeChannel)
        
        // Broadcast over mesh immediately with critical priority
        meshService.sendMessage(sosMessage, mentions: [], messageID: message.id, timestamp: message.timestamp, priority: .critical, messageType: .sos)
    }
    ...
    func blockGeohashUser(pubkeyHexLowercased: String, displayName: String) {
        ...
        if let gh = currentGeohash {
            let predicate: (SafeRelayMessage) -> Bool = { [self] msg in
    ...
    func removeMessage(withID messageID: String, cleanupFile: Bool = false) {
        var removedMessage: SafeRelayMessage?
    ...
    func addLocalPrivateSystemMessage(_ content: String, to peerID: PeerID) {
        let systemMessage = SafeRelayMessage(
            sender: "system",
            content: content,
            timestamp: Date(),
            isRelay: false,
    ...
    @objc private func userDidTakeScreenshot() {
        ...
        if let peerID = selectedPrivateChatPeer {
            ...
            let localNotification = SafeRelayMessage(
                sender: "system",
    ...
    @MainActor
    func getPrivateChatMessages(for peerID: PeerID) -> [SafeRelayMessage] {
        var combined: [SafeRelayMessage] = []
        ...
        var bestByID: [String: SafeRelayMessage] = [:]
        for msg in combined {
    ...
    func panicClearAllData() {
        ...
        userDefaults.removeObject(forKey: "SafeRelay.noiseIdentityKey")
        userDefaults.removeObject(forKey: "SafeRelay.messageRetentionKey")
    ...
    @MainActor
    func formatMessageAsText(_ message: SafeRelayMessage, colorScheme: ColorScheme) -> AttributedString {
        ...
        if message.sender != "system" {
            ...
            senderStyle.font = .SafeRelaySystem(size: 14, weight: fontWeight, design: .monospaced)
            // Make sender clickable: encode senderPeerID into a custom URL
            if let spid = message.senderPeerID, let url = URL(string: "SafeRelay://user/\(spid.toPercentEncoded())") {
            ...
            if (content.count > 4000 || content.hasVeryLongToken(threshold: 1024)) && !containsCashuEarly {
                ...
                plainStyle.font = isSelf
                    ? .SafeRelaySystem(size: 14, weight: .bold, design: .monospaced)
                    : .SafeRelaySystem(size: 14, design: .monospaced)
            ...
            for (range, type) in allMatches {
                ...
                if let nsRange = Range(range, in: content) {
                    ...
                    if type == "mention" {
                        ...
                        mentionStyle.font = .SafeRelaySystem(size: 14, weight: isSelf ? .bold : .semibold, design: .monospaced)
                        ...
                        if type == "hashtag" {
                            // If the hashtag is a valid geohash, make it tappable (SafeRelay://geohash/<gh>)
                            ...
                            if isGeohash && !attachedToMention && standalone, let url = URL(string: "SafeRelay://geohash/\(token)") {
                                tagStyle.link = url
                            ...
                            matchStyle.font = .SafeRelaySystem(size: 14, weight: isSelf ? .bold : .semibold, design: .monospaced)
                            if type == "url" {
            ...
            timestampStyle.font = .SafeRelaySystem(size: 10, design: .monospaced)
            ...
            contentStyle.font = .SafeRelaySystem(size: 12, design: .monospaced).italic()
    ...
    @MainActor
    func formatMessageHeader(_ message: SafeRelayMessage, colorScheme: ColorScheme) -> AttributedString {
        let isSelf: Bool = {
            if let spid = message.senderPeerID {
        ...
        if message.sender == "system" {
            ...
            style.font = .SafeRelaySystem(size: 14, weight: .medium, design: .monospaced)
        ...
        var senderStyle = AttributeContainer()
        senderStyle.foregroundColor = baseColor
        senderStyle.font = .SafeRelaySystem(size: 14, weight: isSelf ? .bold : .medium, design: .monospaced)
        if let spid = message.senderPeerID,
           let url = URL(string: "SafeRelay://user/\(spid.id.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? spid.id)") {
            senderStyle.link = url
        }
    ...
    @MainActor
    private func peerColor(for message: SafeRelayMessage, isDark: Bool) -> Color {
        if let spid = message.senderPeerID {
            if spid.isGeoChat || spid.isGeoDM {
    ...
    @MainActor
    func isSelfMessage(_ message: SafeRelayMessage) -> Bool {
        if let spid = message.senderPeerID {
            // In geohash channels, compare against our per-geohash nostr short ID
            if case .location(let ch) = activeChannel, spid.isGeoChat {
                let myGeo: NostrIdentity? = {
    ...
    @MainActor
    func senderColor(for message: SafeRelayMessage, isDark: Bool) -> Color {
        return peerColor(for: message, isDark: isDark)
    }

    @MainActor
    func peerURL(for peerID: PeerID) -> URL? {
        return URL(string: "SafeRelay://user/\(peerID.toPercentEncoded())")
    }
    ...
    private func addMessage(_ message: SafeRelayMessage) {
    ...
    func getPeer(byID peerID: PeerID) -> SafeRelayPeer? {
    ...
    // MARK: - SafeRelayDelegate Methods
    ...
    func didReceiveMessage(_ message: SafeRelayMessage) {
        Task { @MainActor in
            // Early validation
            ...
            // Trigger Emergency Siren and Notification if it's an SOS
            if message.messageType == .sos && message.sender != self.nickname {
                EmergencySirenManager.shared.playSiren()
                NotificationService.shared.sendSOSNotification(from: message.sender)
                
                // Add to Map if location is provided
                if let geo = message.geoLocation {
                    MapViewModel.shared.addSOS(latitude: geo.latitude, longitude: geo.longitude, sender: message.sender)
                }
            }
    ...
    func didUpdatePeerList(_ peers: [PeerID]) {
        ...
        DispatchQueue.main.async {
            ...
            // Smart notification logic for "SafeRelayters nearby"
            ...
            if meshPeerSet.isEmpty {
                ...
                if !newPeers.isEmpty {
                    ...
                    if Date().timeIntervalSince(self.lastNetworkNotificationTime) >= cooldown {
                        ...
                        SecureLogger.info(
                            "👥 Sent SafeRelayters nearby notification for \(meshPeers.count) mesh peers (new: \(newPeers.count))",
                            category: .session
                        )
                    }
    ...
    func updateMessageDeliveryStatus(_ messageID: String, status: DeliveryStatus) {
        ...
        for (peerID, chatMessages) in privateChats {
            ...
            // Update delivery status directly (SafeRelayMessage is a class/reference type)
    ...
    @MainActor
    func handlePublicMessage(_ message: SafeRelayMessage) {
        ...
        func checkForMentions(_ message: SafeRelayMessage) {    // Determine our acceptable mention token. If any connected peer shares our nickname,
    ...
    func sendHapticFeedback(for message: SafeRelayMessage) {        #if os(iOS)
