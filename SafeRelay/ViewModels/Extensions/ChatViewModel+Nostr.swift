...
extension ChatViewModel {
    ...
    func processNostrMessage(_ giftWrap: NostrEvent) async {
        ...
        do {
            ...
            // Check if it's a BitChat packet embedded in the content (SafeRelay1:...)
            if content.hasPrefix("SafeRelay1:") {
    ...
    func sendDeliveryAckViaNostrEmbedded(_ message: SafeRelayMessage, wasReadBefore: Bool, senderPubkey: String, key: Data?) {
    ...
    private static func decodeEmbeddedBitChatPacket(from content: String) -> SafeRelayPacket? {
        guard content.hasPrefix("SafeRelay1:") else { return nil }
        let encoded = String(content.dropFirst("SafeRelay1:".count))
        ...
        return SafeRelayPacket.from(packetData)
    }
