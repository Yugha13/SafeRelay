...
struct PublicTimelineStore {
    private var meshTimeline: [SafeRelayMessage] = []
    private var geohashTimelines: [String: [SafeRelayMessage]] = [:]
    ...
    mutating func append(_ message: SafeRelayMessage, to channel: ChannelID) {
        switch channel {
        case .mesh:
    ...
    mutating func append(_ message: SafeRelayMessage, toGeohash geohash: String) {
    ...
    mutating func appendIfAbsent(_ message: SafeRelayMessage, toGeohash geohash: String) -> Bool {
        var timeline = geohashTimelines[geohash] ?? []
    ...
    mutating func messages(for channel: ChannelID) -> [SafeRelayMessage] {
        switch channel {
        case .mesh:
            return meshTimeline
        case .location(let channel):
    ...
    @discardableResult
    mutating func removeMessage(withID id: String) -> SafeRelayMessage? {
    ...
    mutating func removeMessages(in geohash: String, where predicate: (SafeRelayMessage) -> Bool) {
        var timeline = geohashTimelines[geohash] ?? []
        timeline.removeAll(where: predicate)
        geohashTimelines[geohash] = timeline.isEmpty ? nil : timeline
    }

    mutating func mutateGeohash(_ geohash: String, _ transform: (inout [SafeRelayMessage]) -> Void) {
        var timeline = geohashTimelines[geohash] ?? []
        transform(&timeline)
        geohashTimelines[geohash] = timeline.isEmpty ? nil : timeline
    }
    ...
    private func trimGeohashTimelineIfNeeded(_ timeline: inout [SafeRelayMessage]) {
