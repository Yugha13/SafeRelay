...
/// Provides static methods for converting between SafeRelayPacket objects and
...
struct BinaryProtocol {
    ...
    // Encode SafeRelayPacket to binary format
    static func encode(_ packet: SafeRelayPacket, padding: Bool = true) -> Data? {
    ...
    // Decode binary data to SafeRelayPacket
    static func decode(_ data: Data) -> SafeRelayPacket? {
    ...
    private static func decodeCore(_ raw: Data) -> SafeRelayPacket? {
        guard raw.count >= v1HeaderSize + senderIDSize else { return nil }

        return raw.withUnsafeBytes { (buf: UnsafeRawBufferPointer) -> SafeRelayPacket? in
